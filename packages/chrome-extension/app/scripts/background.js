      ...
      try {
        ...
        if (activeTab && activeTab.url) {
          ...
          if (!activeTab.url.startsWith('http://ipfs.caralabs.me')) {
            if (
              activeTab.url.startsWith('http://w3.') ||
              ...
              (activeTab.url.startsWith('https://www.google.com/search') &&
                activeTab.url.includes('?q=w3.')) ||
              activeTab.url.includes('?q=web3.') ||
            ...
            if (topDomain && domain) {
              const result = await contract.getStorageHashByDomain(
                domain,
                topDomain
              );

              const url = result
                ? `${storagePrefix}${result}/`
                : `${storagePrefix}${notFoundHash}/`;
        ...
        chrome.tabs.update(
          tabs[0].id,
          { url: storagePrefix + notFoundHash + '/' },
          tab => {
            chrome.tabs.onUpdated.addListener(function listener(
              tabId,
              changeInfo
