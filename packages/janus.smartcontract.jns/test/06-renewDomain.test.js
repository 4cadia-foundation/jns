...
contract('JanusNameService - 06-renewDomain.test.js', (accounts) => {
    ...
    it('renewDomain should be throw if is domain not registered', async() => {
        ...
        await Assert.reverts(
            contractInstance.renewDomain(domainName, topDomainName, { from: ownerAddress }),
    ...
    it('renewDomain should be throw if is not domain owner', async() => {
        ...
        await Assert.reverts(
            contractInstance.renewDomain(domainName, topDomainName, { from: nonOwner }),
    ...
    it('renewDomain success', async() => {
        ...
        const resultRegisterTopDomain = await contractInstance.registerTopDomain(topDomainName, { from: ownerAddress });
        const resultRegisterDomain = await contractInstance.registerDomain(domainName, topDomainName, storageHash, { from: ownerAddress });
        const resultRenew = await contractInstance.renewDomain(domainName, topDomainName, { from: ownerAddress });
    ...
    it('renewDomain success verify expire date', async() => {
        ...
        const resultRenew = await contractInstance.renewDomain(domainName, topDomainName, { from: ownerAddress });
        const renewedDomain = await contractInstance.getDomainByHash(completeDomainHash);
        const registeredDomainExpireDate = new Date(registeredDomain.expires * 1000);
        const reneweddDomainExpireDate = new Date(renewedDomain.expires * 1000);

        Assert.eventEmitted(resultRegisterTopDomain, 'TopDomainRegistered');
        Assert.eventEmitted(resultRegister, 'DomainRegistered');
        Assert.eventEmitted(resultRenew, 'DomainRenewed');
        assert.notEqual(registeredDomainExpireDate.getTime(), reneweddDomainExpireDate.getTime(), 'wrong renew date');
    });
});