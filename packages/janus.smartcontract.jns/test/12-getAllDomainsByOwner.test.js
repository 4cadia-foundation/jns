...
contract('JanusNameService - 12-getAllDomainsByOwner.test.js', (accounts) => {
    ...
    it('return all getAll domains by owner', async() => {
        ...
        const result = await contractInstance.getAllDomainsByOwner({ from: userAddress });
        ...
        assert.equal(resultTopDomainsLength, 4, 'result top domain lenght is wrong');
        ...
        assert.equal(new Date(result.expires[0] * 1000).getFullYear(), new Date().getFullYear() + 1, 'expires0 is wrong');
        assert.equal(new Date(result.expires[1] * 1000).getFullYear(), new Date().getFullYear() + 1, 'expires1 is wrong');
        assert.equal(new Date(result.expires[2] * 1000).getFullYear(), new Date().getFullYear() + 1, 'expires2 is wrong');
    });
});