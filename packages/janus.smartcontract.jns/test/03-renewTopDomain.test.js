...
contract('JanusNameService - 03-renewDomain.test.js', (accounts) => {
    ...
    it('renewTopDomain success', async() => {
        const domainName = 'eth';
        const resultRegister = await contractInstance.registerTopDomain(domainName, { from: ownerAddress });
        const resultRenew = await contractInstance.renewTopDomain(domainName, { from: ownerAddress });
        Assert.eventEmitted(resultRegister, 'TopDomainRegistered');
    ...
    it('renewTopDomain success verify expire date', async() => {
        ...
        const resultRegister = await contractInstance.registerTopDomain(domainName, { from: ownerAddress });
        const registeredDomain = await contractInstance.getTopDomainByHash(domainHash);
    ...
    it('renewTopDomain should be throw if renew limit date is exceeded', async() => {
        ...
        await contractInstance.registerTopDomain(domainName, { from: ownerAddress });
