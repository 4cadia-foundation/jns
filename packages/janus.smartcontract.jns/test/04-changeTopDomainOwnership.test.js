...
contract('JanusNameService - 04-changeDomainOwnership.test.js', (accounts) => {
    ...
    it('changeTopDomainOwnership should be throw if domain is not registered', async () => {
        ...
        await Assert.reverts(
            contractInstance.changeTopDomainOwnership(domainName, futureOwner, { from: ownerAddress }),
    ...
    it('changeTopDomainOwnership should be throw if is not domain owner', async () => {
        ...
        const resultRegister = await contractInstance.registerTopDomain(domainName, { from: ownerAddress });

        Assert.eventEmitted(resultRegister, 'TopDomainRegistered');
        await Assert.reverts(
            contractInstance.changeTopDomainOwnership(domainName, futureOwner, { from: futureOwner }),
    ...
    it('changeTopDomainOwnership success', async () => {
        const domainName = 'eth';
        const resultRegister = await contractInstance.registerTopDomain(domainName, { from: ownerAddress });
        const resultChangeDomainOwnership = await contractInstance.changeTopDomainOwnership(domainName, futureOwner, { from: ownerAddress })

        Assert.eventEmitted(resultRegister, 'TopDomainRegistered');
        Assert.eventEmitted(resultChangeDomainOwnership, 'TopDomainOwnershipChanged');
    });
    ...
    it('changeTopDomainOwnership success verify the new owner', async() => {
        ...
        const resultRegister = await contractInstance.registerTopDomain(domainName, { from: ownerAddress });
        const resultChangeDomainOwnership = await contractInstance.changeTopDomainOwnership(domainName, futureOwner, { from: ownerAddress })
        const changedDomainOwnership = await contractInstance.getTopDomainByHash(domainHash);
